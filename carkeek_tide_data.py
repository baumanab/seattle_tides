...
"""Python code for processing tide data at carkeek park returning a csv to be used in a .js visualization
see tide_tools.py for a more thorough explanation of each function.  Code is intended to be executed from CLI."""
...
def carkeek_tide_processor(begin_dt, end_dt):

    """Function accepts a beginning and end datetime string in the form 'YYYYMMDD mm:ss' which span <= 1 year from
    a current or past date, passing them to the query_builder function. Function creates a .csv file to be used in
    subsequent .js visualizations."""
    ...
    df_sixmin['prev_status'] = df_sixmin[['start_stop', 'shift_neghour_ts']].apply(lambda x: df_sixmin.loc[x[1]],
                                                                                   axis=1).start_stop
...
carkeek_tide_processor(sys.argv[1], sys.argv[2])
